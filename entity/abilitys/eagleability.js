...
const eaglegrab = require('../objects/abilitys/eaglegrab')

function beginfly(entities, player, which) {
    entities[player].startvelocity = 2.5
    let startfly = Date.now() + 2000;
    let newstartfly = setInterval(() => {
        if (!entities[player]) { clearInterval(newstartfly); return };
        if (startfly <= Date.now()) {
            if (entities[player].z > 0) {
                entities[player].goingsky = false

            } else {
                clearInterval(newstartfly)
                ...
                let tempentitygrab = new eaglegrab(entities[player].pos.x, entities[player].pos.y, entities[player].radius)
...
function eagleability(entities, player, which) {
        ...
        let flycheck = setInterval(() => {
            ...
            if (enteredinsky) {//when bar is low
                ...
                if (entities[player].bar.normalbarpercentage <= entities[player].bar.maxbarnormalpercentage / 4 &&
                    ...
                    beginfly(entities, player, which)
...
eagleability.prototype = {}
module.exports = eagleability
